 ...
 * @author Howe Hsiang
 */
public class ExcelPoiWriteUtils {
    ...
    public static void createExcel(String fileDir, String sheetName, String titleRow[]) throws Exception {
        ...
        //判断文件是否存在
        Boolean flag = new ExcelPoiWriteUtils().fileExist(fileDir);
        if (flag){
            deleteExcel(fileDir);
        }
     ...
     * @throws Exception
     */
    public static void writeToExcel(String fileDir, String sheetName, ArrayList<Map<String,String>> mapList) throws Exception {
        //创建workbook
        File file = new File(fileDir);
        try {
            workbook = new HSSFWorkbook(new FileInputStream(file));
